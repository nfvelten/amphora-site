...
<figure>
  <img src="/norn-brain.svg" alt="Diagram showing nine Norn brain lobes. Three signals from the Perception lobe — hunger level, proximity detection, and object identity — converge on a single concept neuron inside the Concept lobe. That neuron fires the Decision lobe, which produces approach behavior." style="width:100%;max-width:760px;" />
  <figcaption>How a concept neuron forms: three signals converge on one node, which fires a decision. The diagram shows only the lobes relevant to this example; other lobes (Noun, Verb, Drive, General Sense, Attention, Stimulus Source) feed into the same architecture via parallel pathways.</figcaption>
</figure>
...
<figure>
  <img src="/norn-brain.svg" alt="Diagrama mostrando nove lobos cerebrais de um Norn. Três sinais do lobo de Percepção — nível de fome, detecção de proximidade e identidade do objeto — convergem em um único neurônio conceitual dentro do Lobo Conceitual. Esse neurônio dispara o Lobo de Decisão, que produz o comportamento de aproximação." style="width:100%;max-width:760px;" />
  <figcaption>Como um neurônio conceitual se forma: três sinais convergem em um nó, que dispara uma decisão. O diagrama mostra apenas os lobos relevantes para este exemplo; os demais (Noun, Verb, Drive, General Sense, Attention, Stimulus Source) alimentam a mesma arquitetura por caminhos paralelos.</figcaption>
</figure>
